...
import { useModalUrlSync } from '@/lib/useModalUrlSync'
...
const ProjectPage = () => {
  ...
  useModalUrlSync(isNewTaskModalOpen, setIsNewTaskModalOpen, 'task')
  useModalUrlSync(isInviteMemberModalOpen, setIsInviteMemberModalOpen, 'invite')
  return (
    <div className="flex flex-col justify-start w-full gap-y-6 p-10">
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center md:flex-col md:items-start sm:items-start sm:flex-col items-start w-full gap-y-6 border-b border-gray-200 pb-4">
        <div>
          <h1 className="text-3xl font-semibold text-secondary-950">
            {project?.name}
          </h1>
        </div>
        ...
        <div className="flex justify-end items-center gap-x-4">
          ...
          <div className="flex items-center gap-x-2">
            <button
              className="bg-primary-600 bg-opacity-40 text-white p-1 rounded-md"
      ...
      <div className="flex flex-col gap-y-4">
        ...
        <p className="text-sm text-gray-600 leading-tight tracking-tight w-1/2 ">
          {project?.description}
        </p>
      </div>
      {/* Project Tasks */}
      <div className="flex flex-col gap-y-4">
        ...
        <div className="flex justify-start items-center gap-x-4">
          <button
            className={`text-md text-secondary-950 px-2 ${
              isActiveTab === 'BOARD'
                ? 'font-semibold border-b-2 border-secondary-950'
                : ''
            }`}
            onClick={() => setIsActiveTab('BOARD')}
          >
            Board
          </button>
          <button
        ...
        {isActiveTab === 'GRAPH' && <Graph id={id} />}
        {isActiveTab === 'LIST' && <ListView id={id} />}
      </div>
    </div>
  )
}
