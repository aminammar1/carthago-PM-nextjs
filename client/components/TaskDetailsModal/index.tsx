...
export const TaskDetailsModal = () => {
  ...
  // Add modal URL sync for task details
  useModalUrlSync(
    isTaskDetailsModalOpen,
    (open) => (open ? null : dispatch(toggleTaskDetailsModalClose())),
    'taskdetails'
  )

  const handleAssignUsers = async (e: React.ChangeEvent<HTMLFormElement>) => {
    e.preventDefault()
    setError('')
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-col w-full gap-5">
            ...
            <form onSubmit={handleAssignUsers} className="flex flex-col gap-2">
              ...
              >
                {assignLoading ? 'Assigning...' : 'Assign users'}
              </button>
            </form>
          </div>
        </div>
