...
export default function NewTaskModal({ projectId, isOpen, onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(error?.data?.error || 'Error creating task')
      setSuccess(false)
    }
  }
        ...
        >
          ...
          <div className="w-full">
            <Select
              ...
              name="dependencies"
              options={
                taskOptions ? taskOptions : [{ value: 0, label: 'no tasks' }]
              }
          ...
          <button
            type="submit"
            className={`w-full rounded bg-orange-500 hover:bg-orange-600 text-white p-2 shadow-sm  ${
              isLoading || !isFormValid() ? 'opacity-40 cursor-not-allowed' : ''
            }`}
