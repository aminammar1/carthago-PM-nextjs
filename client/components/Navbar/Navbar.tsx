...
function Navbar() {
  ...
  const handleSearch = () => {
    if (searchValue.trim()) {
      router.push(
        `/tasks?page=1&search=${encodeURIComponent(searchValue.trim())}`
      )
      setSearchValue('')
    }
  }
              ...
              {dropDownIsOpen && (
                <div className="absolute top-5 right-0 w-40 bg-white shadow rounded-md p-2 border border-gray-200 flex flex-col gap-y-2 z-50">
                  ...
                  <div className="cursor-pointer border-b border-gray-500 w-full">
                    <a href="/profile">
                      <p className="text-sm  text-gray-500  mb-1 hover:text-orange-600 hover:bg-orange-50 rounded-md px-2 py-1">
                        Profile
                      </p>
                    </a>
                  </div>
